...
@router.post(
    path="/completions/async", status_code=status.HTTP_201_CREATED, response_model=Task, summary=""
)
async def create_chat_completion_async(
    ...
    background_tasks.add_task(
        broker.publish, TaskProcess(id=task.id, user_message=user_message), queue="pending_tasks"
    )
...
@router.get(
    path="/tasks/{task_id}", status_code=status.HTTP_200_OK, response_model=Task, summary=""
)

...
async def get_conversation_history(
    ...
    key = build_conversation_history_key(config["configurable"]["chat_id"])  # pyright: ignore[reportTypedDictNotRequiredAccess, reportOptionalSubscript]
    messages = await redis.lrange(key, 0, -1)  # pyright: ignore[reportGeneralTypeIssues]
...
async def cache_conversation_history(state: State, config: RunnableConfig | None = None) -> State:
